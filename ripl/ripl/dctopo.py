...
class FatTreeTopo1(StructuredTopo):
    ...
    def __init__(self, k=4, speed=1.0):
        ...
        self.k = k
        self.id_gen = FatTreeTopo1.FatTreeNodeID
        self.numPods = k
        self.aggPerPod = k / 2
...
class FatTreeTopo(StructuredTopo):
    ...
    def __init__(self, k=4, r=1, speed=1.0):
        '''Init.
...
class BCubeTopo(StructuredTopo):
    ...
    class BCubeNodeID(NodeID):
        ...
        def __init__(self, t=0, fir=0, sec=0, dpid=None, name=None):
            ...
            if dpid:
                ...
                self.fir = fir
                self.sec = sec
                self.dpid = (self.type << 16) + (fir << 8) + sec
            else:
                self.type = t
                self.fir = fir
                self.sec = sec
        ...
        def mac_str(self):
            '''Return MAC string'''
            return "00:00:00:%02x:%02x:%02x" % (self.type, self.fir, self.sec)

        def ip_str(self):
            '''Return IP string'''
            return "10.%i.%i.%i" % (self.type, self.fir, self.sec)
    ...
    def __init__(self, k=1, n=4):
        ...
        @param k levels
        @param n servers per BCube0
        @param speed bandwidth in Gbps
        '''
        Topo.__init__(self)
        self.n = n
        self.k = k
        self.id_gen = BCubeTopo.BCubeNodeID
        n_hosts = n**(k + 1)
        # add host
