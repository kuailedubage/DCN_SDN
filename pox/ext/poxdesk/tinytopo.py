...
class TinyTopo (messenger.ChannelBot):
  ...
  def _handle_HostStatus_HostUp(self, event):
    args = [ int( arg ) for arg in event.ip.split( '.' ) ]
    host = 'h'+str(( args[1] << 16 ) | ( args[2] << 8 ) | args[3])

    switch = event.dpid

    log.info('handle host up!')
    self.hosts.add(host)
