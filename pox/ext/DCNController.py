...
def _calc_paths():
    ...
    for k in sws:
        for i in sws:
            for j in sws:
                if path_map[i][k][0][0] is not None:
                    if path_map[k][j][0][0] is not None:
                        # print '*********************************************************'
                        # print i, '---', k, '---', path_map[i][k][0][0]
                        # print k, '---', j, '---', path_map[k][j][0][0]

                        # i -> k -> j exists
                        ikj_dist = path_map[i][k][0][0] + path_map[k][j][0][0]
                        if path_map[i][j][0][0] is None or ikj_dist < path_map[i][j][0][0]:
                            # i -> k -> j is better than existing
                            # path_map[i][j] = [(None, None)]
                            path_map[i][j] = [(ikj_dist, k)]
                        if ikj_dist == path_map[i][j][0][0] and k not in [i, j, path_map[i][j][0][1]]:
                            path_map[i][j].append((ikj_dist, k))
...
def _get_one_raw_path(src, dst):
    ...
    if src is dst:
        # We're here!
        return []
    if path_map[src][dst][0][0] is None:
        # Not connected
        return None
    intermediate = path_map[src][dst][0][1]
...
def _get_multi_raw_path(src, dst):
    """
    ...
    if path_map[src][dst][0][0] is None:
        ...
        return None
    multi_path = []
    for distance, intermediate in path_map[src][dst]:
        if intermediate is None:
            # Directly connected
            return [[]]
        for path1 in _get_multi_raw_path(src, intermediate):
            for path2 in _get_multi_raw_path(intermediate, dst):
...
def _get_path(src, dst, first_port, final_port):
    ...
    if src == dst:
        ...
        # print '*************', src, '******* ', dst
        # print _get_multi_raw_path(src, dst)
        # TODO choice path
        path = _get_multi_raw_path(src, dst)[0]
        if path is None:
            return None
        log.info('********ALL PATHs********')
        for p in _get_multi_raw_path(src, dst):
    ...
    # print src, '---', dst, '---', path
    # Now add the ports
    r = []  # a list of (node,in_port,out_port)
    in_port = first_port
...
class WaitingPath(object):
    ...
    def notify(self, event):
        ...
        if len(self.xids) == 0:
            ...
            core.DCNController.raiseEvent(PathInstalled(self.path))
...
class PathInstalled(Event):
    ...
    def __init__(self, path):
        self.path = path
        log.info('PathInstalled: %s' % self.path)


def dc_dpid_to_str(dpid):
    """
    Parse dpid to switch name in mininet
    """
    if topotype == 'bc':
        # Bcube topo name parser
        type = (dpid & 0xff0000) >> 16
        fir = (dpid & 0xff00) >> 8
        sec = (dpid & 0xff)
        s = 'unknow'
        if type == 1:
            ...
            s = 'r'
        return "%s%i_%i" % (s, fir, sec)

    if topotype == 'ft':
        # fattree topo name parser
        type = (dpid & 0xff00) >> 8
        ids = (dpid & 0xff)
        s = 'unknow'
        if type == 3:
...
class Switch(EventMixin):
    ...
    def __repr__(self):
        return dc_dpid_to_str(self.dpid)
    ...
    def install_path(self, dst_sw, last_port, match, event):
        ...
        # log.info("Installing path for %s -> %s %04x %s",
        #          match.dl_src, match.dl_dst, match.dl_type, p)
    ...
    def _handle_PacketIn(self, event):
            ...
            if packet.find('arp'):
                ...
                if packet.effective_ethertype == packet.LLDP_TYPE or packet.find('ipv6'):
                    drop()
                    return
                if oldloc is None:
                    if packet.src.is_multicast is False:
                    ...
                    if core.openflow_discovery.is_edge_port(loc[0].dpid, loc[1]):
                        ...
                        if packet.src.is_multicast is False:
                            mac_map[packet.src] = loc  # Learn position for ethaddr
                            log.debug("Learned %s at %s.%i", packet.src, loc[0], loc[1])
                    elif packet.dst.is_multicast is False:
...
class DCNController(EventMixin):
    ...
    def __init__(self):
        ...
        core.listen_to_dependencies(self, listen_args={'openflow': {'priority': 0}})  # TODO
...
def launch(topo=None):
    ...
    # time.sleep(10)
    # def foo():
    core.registerNew(DCNController)
